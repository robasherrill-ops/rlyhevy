...
- `TUTSelector` — Picker for exercise Type (—/Con/Ecc/Hold/Multi) + Duration (2s/3s when type ≠ none)
...
- `TUT_TYPE_LABEL`: `{ none:'—', concentric:'Con', eccentric:'Ecc', hold:'Hold', multiple:'Multi' }`
...
**Reps input:**
- `onChange` → `updateRepsLocal(weId, setId, val)` — updates state only (allows free typing)
...
5. **Type (formerly TUT)** — appears as `tut_type`, `tut_tension`, `tut_duration` in DB/code. UI now calls it "Type". Values: `none`, `concentric`, `eccentric`, `hold`, `multiple`. Duration is 2s or 3s.
